...
- [ ] **M9 Author ingest — make the author page work for any author.** The author page is already a generic parameterized template (`author/[oaid]` + `getAuthor`), but the `authors`/`author_works` tables hold exactly one seeded author (Ulrich), so every other ID returns "no record." Build `pipeline/author_ingest.py` (the author analog of `pull_cohort.py`): given an OpenAlex author ID or ORCID, pull the author entity into `authors`, pull their works and populate `author_works`, and upsert those works into `works`/`qal_records` (QaL where they fall in a calibrated seed community; calibration-pending / universal-layer otherwise). Caveat: an arbitrary author's papers span many subfields we haven't pulled, so the page is only fully meaningful for authors whose output sits largely inside the seed communities — until the V1.0 full-corpus store lands. Pairs with the spec's deferred ORCID-claim/curate layer. Keep the no-single-author-score rule (per-paper distribution only).
