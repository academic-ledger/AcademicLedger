...
- [ ] **U9 Vintage dropdown has gaps (2024, 2025, … missing).** The "Vintage (since)" `<select>` in `ExploreClient` is populated from the *union of years present in the loaded results* (the `years` state accrued from up to 500 returned rows), so any year with no sampled record never appears — currently 2025, 2024, 2018, 2017 are absent. Fix: render a **continuous year range** (e.g. current year `AS_OF` down to a floor like 2000, or down to the min year seen) so every vintage is selectable regardless of which records happened to load. Drop the result-driven `years` accumulation for the dropdown.
