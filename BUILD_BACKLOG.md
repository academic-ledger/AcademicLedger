...
- [ ] **U8 Live search fallback for explore.** Explore search reads only the ingested set, and 1803/1802 are 10k/cohort samples — so famous authors outside the sample (e.g. Cachon) return zero hits, and truncated bylines hide non-first co-authors. Extend the on-the-fly model to search: when the local explore query is empty/thin, fall back to a **live OpenAlex search** for matching works/authors and render them with the universal layer (observed percentile + calibration-pending), like the paper page. Caveat: OpenAlex search/list endpoints are quota-bound (unlike single-entity fetch), so cache results. Shares the live-fetch plumbing with M9 but is about *discovery* (find any paper/author), distinct from M9's author-page construction.
