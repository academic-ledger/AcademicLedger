...
- Field granularity: settled. The official scoring reference class is the synthetic field, a vintage-matched blend of (subfield, year) cohorts whose weights come from the focal paper's references early (research-front, recency-weighted) and migrate to the co-citation community as citations accrue (Sections 3 and 5); the OpenAlex Decision Sciences subfields covering the department define the seed calibration cohort (Section 5).
