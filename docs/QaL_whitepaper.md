...
Every QaL is relative to an explicit reference class. The official one is a **synthetic field**: a vintage-matched blend of (subfield, year) cohorts whose mixture weights describe the focal paper's intellectual community, with the percentile taken over the blended full population (Section 4). A blend rather than a single subject category is used because single-field labels misclassify interdisciplinary work badly, and field-normalization is the central difficulty in citation indicators (Waltman, 2016); the gold-standard version of this idea is algorithmic citation-network field delineation (Waltman & van Eck, 2012), which OpenAlex topics approximate. **Vintage** is the publication year. The official number is fixed so the reference class cannot be shopped; other classes are offered only for exploration.

A word on the units, since the construction rests on them. OpenAlex organizes the literature in a four-level hierarchy: 4 domains, 26 fields, 252 subfields, and about 4,500 topics. The top three levels are the journal-era Scopus classification; the topics are the new layer, citation-network clusters that each map up into exactly one subfield. For example, the field *Decision Sciences* contains the subfields *Management Science & Operations Research* and *General Decision Sciences*, which in turn contain fine topics such as *Advanced Bandit Algorithms* and *Decision-Making and Behavioral Economics*. QaL operates at the **subfield** level: the synthetic field is a weighted blend of subfield cohorts, and everything we precompute and rank against is per subfield, not per topic.
...
These two stages need a floor under them, because a robust construction must place *any* paper, and a working paper fresh on SSRN often has neither: OpenAlex may hold no bibliography for it, and it has no citers yet. Rather than fall straight back to OpenAlex's single primary label — the very misclassification the synthetic field exists to correct — the construction descends one more rung, to a **content-and-authorship prior**. It blends what the paper is about (its own OpenAlex topic assignment, weighted by topic score) with what its authors actually publish on (the recency-weighted subfield mixtures of their recent bodies of work, each author normalized to equal weight so that a prolific co-author does not dominate). This recovers a genuine, multi-subfield community at the moment of posting: an AI-and-management working paper resolves to a blend across safety research, artificial intelligence, information systems, and economics rather than a lone "safety research" label. The single primary subfield is used only as the final fallback, when even that prior is empty. The construction is therefore robust in the operational sense that matters — every indexed paper, whether a decade-old article, one cited twice last month, or one posted minutes ago with no references and no citations, is placed against a blended community rather than a single guessed field — and which rung supplied the weights (references, co-citation, or author prior) is reported with the estimate so the placement can be audited.

In practice the weights need only the references' subfields. Each referenced work already carries one, since its primary topic maps to a single subfield, so we read that subfield, weight it by the reference's recency, and sum. The ~4,500 topics are never enumerated for the weighting; they are the machinery by which OpenAlex assigns each work its subfield, plus a convenient way to show a reader the paper's neighborhood. Finer, topic-level resolution would matter only if we ranked against topic cohorts, which we do not, because those cohorts are too thin to give stable percentiles or calibration.
...
What we precompute is small and concrete: for each (subfield, year) cohort we store a table giving the citation count at each percentile, the empirical CDF, so a paper's standing is a lookup and interpolation rather than a fresh scan. The table also makes the shape of a field visible. In Management Science & Operations Research, 2015, about 54% of the roughly 43,000 papers have zero citations, so the bottom half of the percentile scale is a single tied value, a paper needs only a few citations to clear the median, and the top percentiles then climb steeply into the heavy tail. Those tables, one per (subfield, year), plus the per-subfield calibration, are essentially all that has to be precomputed; the per-paper weighting and ranking are done on the fly.
